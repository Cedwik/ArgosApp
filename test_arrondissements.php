...
<p>Recherche par arrondissement :</p>
<!--formulaire pour permettre à l'utilisateur d'entrer les mots clés de sa recherche -->
<form action="test_arrondissements.php" method="Post" />
...
$query = mysql_query("SELECT id, arrondissement FROM retro_lieu WHERE arrondissement LIKE '%$recherche%' ORDER BY arrondissement") or die (mysql_error()); // la requête, que vous devez maintenant comprendre ;
...
<? if($nb_resultats > 1) { echo 'arrondissements trouvés'; echo ' '; } else { echo 'arrondissement trouvé'; echo ' '; } // on vérifie le nombre de résultats pour orthographier correctement. 
?>
<br/>
<br/>
<?
...
{
?>
<a href="fonction.php?id=<? echo $donnees['id']; ?>"><? echo $donnees['arrondissement']; ?></a><br/>
<?
} // fin de la boucle
?><br/>
...
<p>Nous n'avons trouvé aucun résultat pour votre recherche "<? echo $_POST['recherche']; ?>". <a href="test_arrondissements.php">Réessayez</a> avec autre chose.</p>
<?
